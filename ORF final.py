...
list_of_genes = list(SeqIO.parse("F:\study\Computers\Anup bhaiya ORF program\orf_genomic_1000_all.fasta", "fasta"))
...
output_text_file = open("F:\study\Computers\Anup bhaiya ORF program\ORF output.txt",'w')

output_text_file.write('{:<5}    {:<9}    {:<15}    {:<14}    {:<5}    {:<17}    {:<14}    {:<23}    {:<10}    {:<5}\n'.format('S.No.', 'Gene ID', 'Position of TTG', 'Context of TTG', 'I\O', 'Peptide', 'Peptide length', 'Is it a Kozak Sequence?', 'Is it uORF', 'oORF'))
for i in range(0,len(list_of_object_genes)):
    if len(list_of_object_genes[i].position()) > 1:
#        output_text_file.write('{:<5}    {:<9}    {:<15}    {:<14}    {:<5}    {:<17}    {:<14}    {:<23}    {:<10}\n'.format(i+1, list_of_object_genes[i].gene_id(), list_of_object_genes[i].position()[0], list_of_object_genes[i].context()[0], list_of_object_genes[i].IO()[0], list_of_object_genes[i].translated_peptide_sequence()[0], list_of_object_genes[i].peptide_length()[0], list_of_object_genes[i].isitkozak()[0], list_of_object_genes[i].isuORF()[0]))
        for k in range(0,len(list_of_object_genes[i].position())):
            output_text_file.write('{:<5}    {:<9}    {:<15}    {:<14}    {:<5}    {:<17}    {:<14}    {:<23}    {:<10}    {:<5}\n'.format(i+1 ,list_of_object_genes[i].gene_id() , list_of_object_genes[i].position()[k], list_of_object_genes[i].context()[k], list_of_object_genes[i].IO()[k], list_of_object_genes[i].translated_peptide_sequence()[k], list_of_object_genes[i].peptide_length()[k], list_of_object_genes[i].isitkozak()[k], list_of_object_genes[i].isuORF()[k], list_of_object_genes[i].oORF()[k]))
    elif len(list_of_object_genes[i].position()) == 1:
        output_text_file.write('{:<5}    {:<9}    {:<15}    {:<14}    {:<5}    {:<17}    {:<14}    {:<23}    {:<10}    {:<5}\n'.format(i+1, list_of_object_genes[i].gene_id(), list_of_object_genes[i].position()[0], list_of_object_genes[i].context()[0], list_of_object_genes[i].IO()[0], list_of_object_genes[i].translated_peptide_sequence()[0], list_of_object_genes[i].peptide_length()[0], list_of_object_genes[i].isitkozak()[0], list_of_object_genes[i].isuORF()[0], list_of_object_genes[i].oORF()[0]))            #finds position of 'TTG' -50 nucleotides upstream
    else:
        output_text_file.write('{:<5}    {:<9}    {:<15}    {:<14}    {:<5}    {:<17}    {:<14}    {:<23}    {:<10}    {:<5}\n'.format(i+1, list_of_object_genes[i].gene_id(), '--', '--', '--', '--', '--', '--', '--', '--'))    

output_text_file.close()

fastafile = open("F:\study\Computers\Anup bhaiya ORF program\infast.fasta",'w')
recordobject=[]
for i in range(0, len(list_of_object_genes)):
    for k in range(0,len(list_of_object_genes[i].position())):
